...
export default class QuoteWg extends React.Component {
	...
	shouldComponentUpdate(nextProps, nextState) {
		return (
			nextProps.data.author != nextState.data.author ||
	...
	render() {
		const {
			data: { text, position, author },
			type
		} = this.props;

		return (
			<div className={cx('widget', 'widget--quote', `widget--${type.replace('_', '-')}`)} onClick={this.clickHandler}>
